...
function buildMarkdownDecorationRanges(doc: vscode.TextDocument, selection: vscode.Selection | undefined, context: vscode.ExtensionContext) {
    if (!decorationsBundle) {
        decorationsBundle = createDecorationTypes(context);
    }
    const { anchorTextDecoration, anchorTextActiveDecoration, linkTextDecoration, linkTextActiveDecoration, hiddenDecoration } = decorationsBundle;
    ...
    const anchorMatches = scanMarkdownAnchorMatches(doc);
    ...
    const linkMatches = scanMarkdownLinkMatches(doc);
